...
export function FloatingBalloons({ isVisible }: FloatingBalloonsProps) {
  ...
  useEffect(() => {
    ...
    const initialBalloons: Balloon[] = [
      {
        id: 1,
        x: 20 + Math.random() * 60, // Posición horizontal aleatoria (20-80%)
        y: Math.random() * 50, // Posición vertical aleatoria (0-50%)
        scale: 0.6 + Math.random() * 0.4,
        rotation: -10 + Math.random() * 20,
        image: "/images/leon-globo.png",
        speed: 0.5 + Math.random() * 0.5,
      },
      {
        id: 2,
        x: 20 + Math.random() * 60, // Posición horizontal aleatoria (20-80%)
        y: 50 + Math.random() * 50, // Posición vertical aleatoria (50-100%)
        scale: 0.6 + Math.random() * 0.4,
        rotation: -10 + Math.random() * 20,
        image: "/images/tigre-globo.png",
        speed: 0.5 + Math.random() * 0.5,
      },
    ]
      ...
      {balloons.map((balloon) => (
        ...
        >
          <Image
            ...
            width={120}
            height={120}
            className="h-auto w-auto max-w-[120px] select-none"
            priority
          />
        </motion.div>
      ))}
